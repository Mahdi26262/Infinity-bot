...
# برای گزارش‌گیری خودکار خلاصه‌ی هر ۱۰ بیانیه با ChatGPT.
# مقدارش از GitHub Secret به‌نام OPENAI_API_KEY خونده میشه.
...
def _ask_chatgpt_sync(prompt):
    """درخواست synchronous (بلاک‌کننده) به API چت‌جی‌پی‌تی. هیچ‌وقت
    مستقیم توی event loop اصلی صداش نزن — همیشه از طریق
    asyncio.to_thread تا ربات وسط این درخواست فریز نشه."""
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "تو یه خبرنگار تحلیلگر بازی جنگی «Infinity War» "
                            "هستی. بیانیه‌های رسمی کشورهای مختلف رو می‌خونی و "
                            "یه گزارش خلاصه‌ی کوتاه، بی‌طرف و خوانا به فارسی "
                            "می‌نویسی که مهم‌ترین تحولات، تهدیدها و تنش‌های بین "
                            "کشورها رو در چند پاراگراف کوتاه جمع‌بندی می‌کنه."
                        ),
                    },
...
async def summarize_last_announcements(batch_size=10):
    """آخرین batch_size تا بیانیه رو می‌خونه، با ChatGPT خلاصه‌ش می‌کنه
    و گزارش رو تو همون کانال بازی پست می‌کنه."""
    ...
    prompt = "بیانیه‌های اخیر:\n\n"
    for row in rows:
        prompt += f"— کشور {row['name']}:\n{row['text']}\n\n"

    summary = await asyncio.to_thread(_ask_chatgpt_sync, prompt)
    ...
    report = (
        "📊 گزارش خلاصه — ۱۰ بیانیه‌ی اخیر\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{summary}\n\n"
        f"𝐈𝐍𝐅𝐈𝐍𝐈𝐓𝐘 𝐖𝐀𝐑 | @{CHANNEL}"
    )
